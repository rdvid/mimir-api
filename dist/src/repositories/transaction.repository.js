...
const applyFilters = (query, userId, filters) => {
...
export const transactionRepository = {
    ...
    async list(userId, filters) {
        ...
        const query = applyFilters(db(TRANSACTIONS)
            .select(`${TRANSACTIONS}.*`, `${CATEGORIES}.name as category_name`)
            .leftJoin(CATEGORIES, `${CATEGORIES}.id`, `${TRANSACTIONS}.category_id`), userId, filters);
    ...
    async getPeriodSummary(userId, filters) {
        const query = applyFilters(db(TRANSACTIONS), userId, filters);
        const result = await query
            .select(db.raw(`COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as total_income`), db.raw(`COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as total_expense`), db.raw('COUNT(*)::int as count'))
            .first();
    ...
    async getCategorySummary(userId, filters) {
        const query = applyFilters(db(TRANSACTIONS)
            .select(`${TRANSACTIONS}.category_id`, `${CATEGORIES}.name`, db.raw('COALESCE(SUM(amount), 0) as total'), db.raw('COUNT(*)::int as count'))
            .leftJoin(CATEGORIES, `${CATEGORIES}.id`, `${TRANSACTIONS}.category_id`), userId, filters);
        const rows = await query
    ...
    async getMonthlySummary(userId, year, type) {
        ...
        const rows = await query
            .select(db.raw(`TO_CHAR(date, 'YYYY-MM') as month`), db.raw(`COALESCE(SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END), 0) as total_income`), db.raw(`COALESCE(SUM(CASE WHEN type = 'expense' THEN amount ELSE 0 END), 0) as total_expense`))
